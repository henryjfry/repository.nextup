...
class PlayerDialogs(xbmc.Player):
	...
	xbmc.log(str(self.next_url)+'NEXT_URL___PLAYER1===>service.next_playlist1', level=xbmc.LOGNOTICE)
	...
	xbmc.log(str(actionArgs)+' actionArgs===SEREN_PLAYER', level=xbmc.LOGNOTICE)
...
#        PlayingNext('playing_next.xml', xbmcaddon.Addon().getAddonInfo('path').decode('utf-8'), actionArgs=next_url).doModal()
	...
	while xbmc.Player().isPlayingVideo()==0:
	        xbmc.log(str(xbmc.Player().isPlayingVideo())+'PLAYER2===>service.next_playlist1', level=xbmc.LOGNOTICE)
		xbmc.sleep(500)

	xbmc.log(str(xbmc.Player().isPlayingVideo())+'PLAYER3===>service.next_playlist1', level=xbmc.LOGNOTICE)
	"""
